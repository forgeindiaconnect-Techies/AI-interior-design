### MARKDOWN
<center>
    <h1> Generating Interior Design (Architectural 100K+) </h1>
    <h3>using  wGAN</h3>
</center>

### MARKDOWN
In this notebook, we will attempt to generate modern interior designs by using a subset of the dataset and the Wasserstein GAN model. Due to time constraints, we will resize the images to a smaller size and use only the grayscale channel to speed up the training process.

<strong>
<ol>
<li><a href="#dcgan">DCGAN Implementation</a></li>
<li><a href="#wgan">wGan Implementation</a></li>
    <li><a href="#results">Evaluating Results</a></li>
</ol>
</strong>

### CODE
# Install required dependencies for running this notebook
!pip install torch torchvision numpy matplotlib wandb pillow tqdm
...
### MARKDOWN
Settings random seed, we will turn off the usage of deterministic algorithms as it for some reason wasn't letting the wGAN train
...
### MARKDOWN
We will filter some images, as we don't have enough time to train on the whole 80k images and also we want to filter them so the model doesn't get confused
...
We need to downscale our data, also convert it to greyscale for easier trainning
...
### MARKDOWN
<h1 id="dcgan">Implementing DCGAN</h1>
...
**Quoted from pytorch.org** 

From the DCGAN paper, the authors specify that all model weights shall be randomly initialized from a Normal distribution with mean=0, stdev=0.02. The weights_init function takes an initialized model as input and reinitializes all convolutional, convolutional-transpose, and batch normalization layers to meet this criteria. This function is applied to the models immediately after initialization.

### MARKDOWN
nn.init.normal_ initializes the weights of the convolutional layer using a normal (Gaussian) distribution with a mean of 0.0 and a standard deviation of 0.02. This is a common practice in GANs (Generative Adversarial Networks) and convolutional networks to stabilize training.
...
### MARKDOWN
We will use binary cross entropy as our loss function, and compile the models using adam optimizer and lr 
...
### MARKDOWN
The model was incredibly unstable, and the results are just a pure noise, dcgan wasn't didn't work for this problem

![](https://i.postimg.cc/T3spdhCh/image.png)

### MARKDOWN
![](https://i.postimg.cc/yNFBMNGM/image.png)
...
### MARKDOWN
nn.BCELoss(): This is the Binary Cross Entropy loss function

torch.randn(64, nz, 1, 1): This generates a batch of 64 random noise vectors from a standard normal distribution (N(0, 1)), where:

- 64 is the batch size.
- nz is the size of the latent space (the dimension of the noise vector input to the generator).
- The shape (nz, 1, 1) is a typical input format expected by a generator in a DCGAN. It represents the latent space vector in 4D shape (batch_size, channels, height, width).
...
### MARKDOWN
# Generating Using WGAN-GP

### MARKDOWN
The key difference for WGAN-GP is in the critic. The output will not have a Sigmoid function, and we will add gradient penalty calculations.

### CODE
wandb.init(project='wgan-architecture-v4')

### MARKDOWN
- z_dim: This is the dimension of the latent vector fed into the generator. The generator will take this random noise and turn it into an image. 
- devide: This means the model will use the GPU for training. If your system supports CUDA, this will greatly accelerate training.
- cur_step: This is a tracking variable for the current step in training. It's useful for logging progress, saving models, and displaying sample outputs.
- crit_cycles: In WGAN, the critic (discriminator) is trained multiple times for each generator update. Training the critic 5 times per generator step helps to stabilize training. This is crucial since WGAN uses the Wasserstein distance, which requires a well-trained critic.
- genlosses: These lists will store the losses of the generator and critic
- show_step: how often to show images
...
### MARKDOWN
# Define Optimizers (Adam 0.00005,0.5,0.999)
...
### MARKDOWN
![](https://i.postimg.cc/3r1TgRzR/Whats-App-Image-2024-10-08-at-16-14-32-15fc3ce8.jpg)

### MARKDOWN
![](https://i.postimg.cc/jS4xkXFH/Whats-App-Image-2024-10-08-at-15-46-11-4b3392ef.jpg)

### MARKDOWN
<center>
<h4> I don't know why but i really like this one </h4>
<img src="https://i.postimg.cc/RFrg67vY/Whats-App-Image-2024-10-08-at-15-42-58-bc8a5449.jpg" width="250" height="250"></center>

### MARKDOWN
![](https://i.postimg.cc/WzsLrLsx/image.png)

### MARKDOWN
Overall, wGAN performed better on this dataset, but i still want to tune the model and change the model architecture.
